    ...
    for (int i = 0; i < NUM_FIXTURES; i++) {
        ...
        if (data->fixtures[i].num_triggers_changed > 0 || data->fixtures[i].num_values_changed > 0) {
            ...
            for (int v = 0; v < NUM_VALUES; v++) {
                if (data->fixtures[i].value_data.values_changed[v]) {
                    buffer[idx++] = v; // index
                    buffer[idx++] = data->fixtures[i].value_data.values[v]; // value
                }
            }
            ...
            for (int t = 0; t < NUM_TRIGGERS; t++) {
                if (data->fixtures[i].trigger_data.triggers_changed[t]) {
                    buffer[idx++] = t; // index
                    buffer[idx++] = data->fixtures[i].trigger_data.triggers[t]; // value
                }
            }
